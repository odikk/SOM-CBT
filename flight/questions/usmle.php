...
if( ( $userdata['session_logged_in'] ) && ( ($userdata['user_level'] == ADMIN) || ($userdata['user_level'] == TEACHER )) ){
	...
	$template->set_filenames(array(	'body' => 'questions/usmle.tpl')	);
	...
	$template->assign_block_vars('usmle',array('FORM_ACTION' => append_sid('usmle.php?ARG1='.$cid.'&ARG2='.$ca.'&ARG3='.$qid.'&lck='.$lock.'&QUIZ='.$qzid),'LINK' =>  'popUp2(\''.append_sid('list_entities.php?action=add&ARG1='.$cid.'&ARG3='.$question['question_id']).'\')',
												  'SRC' => append_sid('usmle_iframe.php?ARG1='.$cid.'&ARG3='.$qid.'&lck='.$lock.'&QUIZ='.$qzid)		
									 ));
